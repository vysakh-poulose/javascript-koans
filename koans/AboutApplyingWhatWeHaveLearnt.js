...
describe("About Applying What We Have Learnt", function () {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
    ...
    const isMushroomPresent = (ingredient) => ingredient === "mushrooms";

    /* solve using filter() & all() / any() */
    productsICanEat = products.filter(
      ({ containsNuts, ingredients }) =>
        !containsNuts && !ingredients.some(isMushroomPresent)
    );
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    const isMultiple = (number) => number % 3 === 0 || number % 5 === 0;

    var sum = _.range(0, 1000).reduce((a, b) => (isMultiple(b) ? a + b : a));
  ...
  it("should count the ingredient occurrence (functional)", function () {
    ...
    products
      ...
      .flat()
      .reduce((_, ingredient) => mapIngredients(ingredient));
